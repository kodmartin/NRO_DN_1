...
int main() {
  ...
  if (!infile.is_open()) {
      std::cerr << "Ne morem odpreti: " << filename << std::endl;
      return 1;
  }
  // preberemo datoteko
  std::ifstream infile;
  ...
  for (int ii=0; ii<2000; ii++)
      ...
      {
          if (ii % 50 == 0) {
              std::cout << "Iteracija: " << ii << std::endl;
          }
          ...
          for (int jj = 0; jj < n; jj++)
          {
              ...
              if (A[jj][jj] == 0.0) {
                  std::cerr << "NAPAKA: A[" << jj << "][" << jj << "] = 0 (deljenje z 0)" << std::endl;
                  return 2;
              }
              ...
              {
                  if (kk != jj) d -= A[jj][kk] * T[kk];
              }

              double newT = d / A[jj][jj];

              // zaščita pred NaN/Inf
              if (!std::isfinite(newT)) {
                  std::cerr << "NAPAKA: T[" << jj << "] ni koncen (NaN/Inf) pri iteraciji " << ii << std::endl;
                  return 3;
              }
...
// Gauss-Seidelova metoda temelji na zaporednem posodabljanju vrednosti, zato njena uporaba v vzporednem okolju, 
// kot je OpenMP, ni ustrezna. Ključni problem je v tem, da se na novo izračunana vrednost takoj uporabi pri nadaljnjih 
// izračunih znotraj iste iteracije. Tako se denimo rezultat v vozlišču T[1] neposredno upošteva pri izračunu 
// vozlišča T[2]. Če bi tak algoritem izvajali vzporedno, bi lahko več niti hkrati dostopalo do istega pomnilniškega
// mesta in vanj zapisovalo različne vrednosti, kar bi povzročilo nekonsistentne rezultate. To lahko vodi v nepravilno 
// nadaljevanje računanja ali celo v izgubo konvergence. Zaradi takšnih podatkovnih odvisnosti Gauss-Seidelove metode 
// ni mogoče varno paralelizirati z OpenMP.
